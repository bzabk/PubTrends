...
def parse_pmid_to_dbidx(response_result: dict[str, Any], pmid: int) -> DatasetLinkDto:
    try:
        db_ids = response_result["linksets"][0]["linksetdbs"][0]["links"]
    except Exception as e:
        raise ParserError(f"Failed to parse bdidx for pmid") from e

    return DatasetLinkDto(pmid=pmid, db_ids=db_ids)


def parse_dataset_summary(response_result: dict[str, Any], db_idx: int) -> DatasetSummaryDto:
    try:
        item = response_result["result"][str(db_idx)]
    except Exception as e:
