...
class ApiClient:
    ...
    async def db_idx_to_info_layer(self,db_idx: int) -> PmData:
        ...
        async with self.semaphore:
            return await self.get_data_from_url(url=ApiClient._BASE_URL_SUMMARY,params=params,
                                                parser=info_from_db_idx_parser,idx=db_idx,
    ...
    async def gse_code_to_overall_design(self,gse_code: str):
        ...
        async with self.semaphore:
            return await self.get_data_from_url(url=ApiClient._BASE_URL_OVERALL_DESIGN,params=params,
