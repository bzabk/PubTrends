...
class PubMedDataRetriever:
    ...
    async def async_call_for_single_pmid(self,pmid_idx):
    ...
    async def get_data_from_url(self,url,params,parser,**kwargs):
    ...
    async def pmid_to_db_idx_layer(self,id: int) -> List[int] | None:
        ...
        async with self.semaphore:
            self.counter += 1
            return await self.get_data_from_url(url=PubMedDataRetriever.BASE_URL_DB_IDX,params=params,parser=pmid_to_db_idx_parser)
    ...
    async def db_idx_to_info_layer(self,db_idx: int) -> PmData:
        ...
        async with self.semaphore:
            self.counter += 1
            return await self.get_data_from_url(url=PubMedDataRetriever.BASE_URL_SUMMARY,params=params,parser=info_from_db_idx_parser)
            ...
            #         pm_data = await info_from_db_idx_parser(response, db_idx)
            #         return pm_data
            #     except Exception as e:
        ...
        async with self.semaphore:
            ...
            for attempt1 in range(1, PubMedDataRetriever.RETRIEVAL_TIMES + 1):
                try:
                    ...
                    response_data = await overall_design_parser(response)
